<template>
  <div class="view-container">
    <el-card>
      <div class="toolbar">
        <el-button type="primary" @click="openForm">新增队员</el-button>
        <el-button @click="refreshData">刷新</el-button>
      </div>
      <el-table :data="members" border>
        <el-table-column prop="memberNo" label="队员编号" />
        <el-table-column prop="memberName" label="队员姓名" />
        <el-table-column prop="phone" label="联系电话" />
        <el-table-column prop="position" label="职位" />
        <el-table-column prop="teamName" label="所属小队" />
        <el-table-column label="操作">
          <template #default="{ row }">
            <el-button size="small" @click="openForm(row)">编辑</el-button>
            <el-button size="small" type="danger" @click="deleteMember(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>

    <el-dialog v-model="formVisible" title="队员信息" width="500px">
      <el-form :model="form" label-width="100px">
        <el-form-item label="队员编号" required>
          <el-input v-model="form.memberNo" />
        </el-form-item>
        <el-form-item label="队员姓名" required>
          <el-input v-model="form.memberName" />
        </el-form-item>
        <el-form-item label="联系电话">
          <el-input v-model="form.phone" />
        </el-form-item>
        <el-form-item label="职位">
          <el-input v-model="form.position" />
        </el-form-item>
        <el-form-item label="所属小队" required>
          <el-select v-model="form.teamId" placeholder="选择小队">
            <el-option v-for="team in teams" :key="team.id" :label="team.teamName" :value="team.id" />
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="formVisible = false">取消</el-button>
        <el-button type="primary" @click="saveMember">保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { ElMessage } from 'element-plus'
import { memberApi, type TeamMember } from '@/api/member'
import { teamApi, type SurveyTeam } from '@/api/team'

const members = ref<TeamMember[]>([])
const teams = ref<SurveyTeam[]>([])
const formVisible = ref(false)
const editingId = ref<number | null>(null)

const form = ref({
  memberNo: '',
  memberName: '',
  phone: '',
  position: '',
  teamId: 0
})

const loadData = async () => {
  members.value = await memberApi.getAll()
  teams.value = await teamApi.getAll()
}

const openForm = (row?: TeamMember) => {
  if (row) {
    editingId.value = row.id
    form.value = {
      memberNo: row.memberNo,
      memberName: row.memberName,
      phone: row.phone,
      position: row.position,
      teamId: row.teamId
    }
  } else {
    editingId.value = null
    form.value = {
      memberNo: '',
      memberName: '',
      phone: '',
      position: '',
      teamId: 0
    }
  }
  formVisible.value = true
}

const saveMember = async () => {
  try {
    if (editingId.value) {
      await memberApi.update(editingId.value, form.value)
      ElMessage.success('更新成功')
    } else {
      await memberApi.create(form.value)
      ElMessage.success('创建成功')
    }
    formVisible.value = false
    await loadData()
  } catch (error: any) {
    ElMessage.error(error.message || '操作失败')
  }
}

const deleteMember = async (id: number) => {
  try {
    await memberApi.delete(id)
    ElMessage.success('删除成功')
    await loadData()
  } catch (error: any) {
    ElMessage.error(error.message || '删除失败')
  }
}

const refreshData = async () => {
  await loadData()
  ElMessage.success('刷新成功')
}

onMounted(loadData)
</script>

<style scoped>
.view-container {
  max-width: 1200px;
}

.toolbar {
  display: flex;
  gap: 10px;
  margin-bottom: 20px;
}
</style>